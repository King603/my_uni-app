<template>
	<view class="calendar">
		...
		<view class="my-flex-row-b my-font-13 my-cl-ye my-pdl-10 my-pdr-10 my-pdb-10">
			<text>本月总额：{{earning.amount}}元</text>
			<text>我的收益：{{earning.proxyAmount}}元</text>
		</view>
	...
	export default {
		...
		methods: {
			getBusinessBill() {
				...
				req.request('GET', data, '/proxy/business-bill/' + this.businessIds[this.select] + '?year=' + this.pickerData.list[
					0][this.pickerData.indexList[0]] + '&&month=' + this.pickerData.list[1][this.pickerData.indexList[1]], '加载中', (
					res) => {
					// console.log("/proxy/business-bill", res)
					if (res.statusCode === 200) {
						this.earning = res.data.data
						this.datePrice=res.data.data.businessBillDailyList
					} else {
						uni.showToast({
