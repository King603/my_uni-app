...
QRCodeAlg.prototype = {
  ...
  setupPositionProbePattern(row, col) {
    for (let y = -1; y <= 7; y++) {
      if (row + y <= -1 || this.moduleCount <= row + y) continue;
      for (let x = -1; x <= 7; x++) {
        if (col + x <= -1 || this.moduleCount <= col + x) continue;
        this.modules[row + y][col + x] = (0 <= y && y <= 6 && (x == 0 || x == 6))
          || (0 <= x && x <= 6 && (y == 0 || y == 6))
          || (2 <= y && y <= 4 && 2 <= x && x <= 4);
      }
    }
...
QRPolynomial.prototype = {
   ...
   * @return {QRPolynomial}   [description]
   */
  multiply(e) {
