					...
					<QSPopup ref="QSPopup11" type="fadeScaleHeightToLowInMiddle">
						<view class="my-bg-white my-rds-10 box-show1">
							<view class="my-flex-row-align-c show2">
								<image src="../../static/hch-position/pinpaiicon2x.png" class="pic"></image>
							...
							<view class=" my-flex-row-align-c my-font-14 my-cl-90" style="margin-top: 37upx;">
								<image src="../../static/hch-position/10213.png" class="pic1"></image>
							...
							<view class="my-flex-row-align-c my-font-14 my-cl-90 ">
								<image src="../../static/hch-position/1023xy.png" class="pic1"></image>
								<view class="my-flex-row icon-pic" style="padding-top: 22upx;">
...
<script>
	...
	export default {
		...
		methods: {
			isshowsite(parentToChildData3){
				let self = this
				if (!parentToChildData3) {
				  return
				}
				self.site = parentToChildData3
			},
			isshowmarkers(parentToChildData2){
				let self = this
				if (!parentToChildData2) {
				  return
				}
				self.markers = parentToChildData2
			},
			isshowstoreData(parentToChildData){
                    let self = this
				    if (!parentToChildData) {
				      return
				    }
				    self.storeData = parentToChildData
			},
			// 点击了门店信息
			storeDesEvn() {
				uni.showToast({
