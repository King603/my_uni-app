	...
	<view class="content">
		...
		<view class="box bg my-flex-row-b">
			<view class="my-flex-row-align-c">
				<view class="avater bder-r-white my-pdlr-15" @click="onGoHome"><image :src="avatarUrl" class="my-hdimg-c-30" mode="aspectFill"></image></view>
...
export default {
	...
	onShow() {
		console.log('logged:'+uni.getStorageSync('logged'));
		console.log('bindPhone:'+uni.getStorageSync('bindPhone'));
		// uni.showModal({
		// 	title:'提示',
		// 	content:'取消授权，可能无法继续租借充电宝，请重新授权。',
		// 	cancelText:'我知道了',
		// 	confirmText:'重新授权'
		// });
		...
		if (this.isLogin == true) {
			login.userLogin().then(
				...
				error => {
					// uni.showModal({
					// 	title:'提示',
					// 	content:'取消授权公开信息，可能无法继续租借充电宝，请重新授权。',
					// 	cancelText:'我知道了',
					// 	confirmText:'重新授权'
					// })
				...
				if (this.bindPhone === false) {
					...
					uni.scanCode({
						success: res => {
							...
							} else if (result.indexOf('/ch/X') > 0) {
								console.log('扫到线充二维码');
								let lineDeviceNo = result.split('/')[result.split('/').length - 1];
								uni.navigateTo({
									url: '/pages/index/lineCharge?lineDeviceNo=' + lineDeviceNo
								});
							} else {
								console.log('扫到充电宝二维码');
