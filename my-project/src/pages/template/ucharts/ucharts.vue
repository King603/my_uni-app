  ...
  methods: {
    getServerData() {
      ...
      uni.request({
        ...
        data: {},
        success: (res) => this.fillData(res.data),
        fail: () => (this.tips = "网络错误，小程序端请检查合法域名"),
        complete() {
          uni.hideLoading();
        },
      });
    ...
    fillData(data) {
      ...
      for (let i = 0; i < Ring.series.length; i++)
        Ring.series[i].format = () => Ring.series[i].name + Ring.series[i].data;
    ...
    showRing(canvasId, chartData) {
      canvasObj[canvasId] = new uCharts({
        ...
        extra: {
          pie: {
            lableWidth: 15,
            ringWidth: 40 * this.pixelRatio, //圆环的宽度
            offsetAngle: 0, //圆环的角度
          },
        },
        background: "#FFFFFF",
    ...
    changeGaugeData() {
      ...
      let newTitleColor;
      for (let i = 0; i < this.serverData.Gauge.categories.length; i++) {
        if (series[0].data <= this.serverData.Gauge.categories[i].value) {
          newTitleColor = this.serverData.Gauge.categories[i].color;
          break;
        }
      }
      ...
      canvasObj.canvasGauge.updateData({
        ...
        categories: this.serverData.Gauge.categories,
        title: {
    ...
    changeData() {
      canvasObj.canvasColumn.updateData({
        series: this.serverData.ColumnB.series,
        categories: this.serverData.ColumnB.categories,
      });
    },
    touchLineA(e) {
      ...
      canvasObj[id].showToolTip(e, {
        format(item, category) {
          return `${category} ${item.name}:${
            typeof item.data === "object" ? item.data.value : item.data
          }`;
        },
      });
    },
