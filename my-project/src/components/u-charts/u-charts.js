...
var util = {
	...
	isSameSign(num1, num2) {
		return Math.abs(num1) === num1 && Math.abs(num2) === num2 || Math.abs(num1) !== num1 && Math.abs(num2) !== num2;
	},
	isSameXCoordinateArea(p1, p2) {
...
function drawPieDataPoints(series, opts, config, context) {
	...
	if (opts.dataLabel !== false && process === 1) {
		...
		if (valid) drawPieText(series, opts, config, context, radius, centerPosition);
	}

	if (process === 1 && opts.type === 'ring') drawRingTitle(opts, config, context, centerPosition);
...
function drawRoseDataPoints(series, opts, config, context) {
	...
	if (config.pieChartLinePadding == 0) config.pieChartLinePadding = roseOption.activeRadius;
	var centerPosition = {
		x: opts.area[3] + (opts.width - opts.area[1] - opts.area[3]) / 2,
		y: opts.area[0] + (opts.height - opts.area[0] - opts.area[2]) / 2
	};
 ...
 * 
 * @param {TouchEvent} e 
 * @param {{format: (item: any) => string}[]} args 
 */
Charts.prototype.showToolTip = function (e, ...args) {
	var option = args.length > 1 && args[0] !== undefined ? args[0] : {};
	var touches = null;
	if (e.changedTouches) {
		touches = e.changedTouches[0];
	} else {
