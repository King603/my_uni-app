<template>
	<view class="content">
		<hchPosition :storeData="storeData" :site="site" :markers="markers" @st="st('QSPopup')"></hchPosition>
		<view class="box bg my-flex-row-b">
			<view class="my-flex-row-align-c">
				<view class="avater my-pdlr-15 bder-r-white" @click="onGoHome"><image :src="avatarUrl" class="my-hdimg-c-30" mode=""></image></view>
				...
				<view class="tab-h2 bg2 my-flex-row-b">
					<view class="my-flex-row-align-c my-pdl-7 my-pdr-15" @click="onRecord">
						<view style="width: 43upx; height: 43upx; border: 3upx solid #FFFFFF; border-radius: 50%;">
							<image class="icon1 my-pdtb-5" src="../../static/hch-position/record.png" mode="scaleToFill"></image>
						</view>
					...
					<view class="my-flex-row-align-c my-pdr-5 my-pdl-15" @click="onSearch">
						...
						<image class="my-mgl-5 icon" src="/static/hch-position/branch.png" mode=""></image>
					</view>
				</view>
			</view>
		</view>
		...
		<QSPopup ref="QSPopup1" sty type="fadeScaleHeightToLowInMiddle">
			<view class="show1 my-bg-white my-rds-10">
				<view class="my-flex-col-c  my-pd-20 my-rds-10">
					...
					<button class="my-bg-F3 my-font-16 my-mgt-18 show4-btn my-cl-white" open-type="getPhoneNumber" @getphonenumber="getPhoneNumber">立即绑定</button>
				</view>
			</view>
		</QSPopup>
	</view>
...
export default {
	data() {
		return {
			...
			markers: [
				...
				// 	latitude: 24.58023061,
				// 	longitude: 118.10388605,
				// 	iconPath: '/static/hch-position/shop.png',
				// 	width: 48,
				// 	height: 48,
				// 	callout: {
				// 		content: "嘉庚体育馆",
	...
	mounted() {
		// console.log((CryptoJS.HmacSHA256(this.b, this.key).toString()).toUpperCase())
	...
	methods: {
		getUrlParamVal(url, paramName) {
			var query = url.toString().split('?')[1];
			...
			req.request('POST', params, '/user/business/map', '加载中', res => {
				...
				if (res.statusCode === 200) {
					...
					this.markers.forEach(res => {
						...
						(res.width = 48), (res.height = 48), (res.latitude = Number(res.addressLatitude));
		...
		getLocal(latitude, longitude) {
			...
			// 		(vm.province = province), (vm.latitude = latitude), (vm.longitude = longitude), (vm.city = city), (vm.site = res.result.address);
			// 		// console.log(vm.site)
			// 		vm.getMap();
			// 	},
			// 	fail: function(res) {
